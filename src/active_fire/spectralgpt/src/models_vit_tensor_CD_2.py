...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.cls_seg = nn.Sequential(
            nn.Conv2d(256, int(num_classes), kernel_size=3, padding=1)
        )
        ...
        self.decoder = FPNHEAD(num_classes=num_classes)
        ...
        if 'seg_classes' in kwargs:
            self.cls_seg = nn.Sequential(
                nn.Conv2d(256, int(kwargs['seg_classes']), kernel_size=3, padding=1),
            )
    ...
    def forward(self, x1):
        ...
        if self.img_size == 256:
            seg1 = seg1.reshape(B, 32, 32, 768).permute(0, 3, 1, 2).contiguous()
        else:
...
class FPNHEAD(nn.Module):
    def __init__(self, channels=2048, out_channels=256, num_classes=13):
        super(FPNHEAD, self).__init__()
        self.PPMHead = PPMHEAD(in_channels=channels, out_channels=out_channels, num_classes=num_classes)
...
def vit_base_patch8_128_channel3(**kwargs):
    model = VisionTransformer(
        img_size=128,
        in_chans=1,
        patch_size=8,
...
def vit_base_patch8_256_channel3(**kwargs):
    model = VisionTransformer(
        img_size=256,
        in_chans=1,
        patch_size=8,
