...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Crop dataset into patches')
    parser.add_argument('--input_path', help='Path to the input dataset')
    parser.add_argument('--output_path', help='Path to the output directory for cropped patches')
    args = parser.parse_args()

    # images = glob(os.path.join(IMAGES_DIR, '**', '*_20m_merged.tif'), recursive=True)
    images = glob(os.path.join(args.input_path, '**', '*.tif'), recursive=True)

    print(f'Num Images: {len(images)}')


    print('Cropping dataset into patches...')
    os.makedirs(args.output_path, exist_ok=True)
    for image in tqdm(images, total=len(images)):
        get_split(image, args.output_path)
