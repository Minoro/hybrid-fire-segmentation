...
class ThermalAnomalyBranchPP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, domain_id: torch.Tensor):
        ...
        if self.num_domains == 1:
            thr_stgi = self.thr_stgi
            thr_acst = self.thr_acst
            thr_sai  = self.thr_sai
            thr_sati = self.thr_sati
            ...
            gamma_fire = self.gamma_fire
            delta_fire = self.delta_fire

            alpha_sati = self.alpha_sati
            beta_sati  = self.beta_sati
            k_contrast = self.k_contrast
...
class LitDTBranchAdvanced(L.LightningModule):
    ...
    def forward(self, x, domain_id=None):
        ...
        dt_out = self.threshold_branch(x, domain_id)
